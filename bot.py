...
def get_git_info(repo_path, target_branch=None):
    try:
        if target_branch:
            # Get the commit object for the specified branch
            repo = git.Repo(repo_path)
            branch_commit = repo.refs['refs/remotes/origin/' + target_branch].commit
            commit = branch_commit.hexsha[:7]  # Just to get the short commit hash
            branch = target_branch
        else:
            # Get the commit object for the current branch
            repo = git.Repo(BOT_DIRECTORY)
            repo.git.fetch()
            commit = repo.head.object.hexsha[:7]  # Just to get the short commit hash
        ...
        logger.error(f"Returning None, None: {e} Type: {type(e).__name__}")
        raise e
...
async def source(ctx):
    await ctx.send(f"Source code available at {GIT_REPO_URL[:-4]} under the Affero GPL-3.0 license.")
